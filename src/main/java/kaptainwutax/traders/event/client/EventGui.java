...
public class EventGui {
	...
	public static void onGuiOpen(GuiOpenEvent event) throws IllegalArgumentException, IllegalAccessException {
		...
		String uuid = merchant.getDisplayName().getStyle().getHoverEvent().getValue().getFormattedText().split("\"")[3];
		
		if(!EntityTrader.UUIDS.contains(uuid))return;

		EntityPlayer player = MINECRAFT.player;
	
		event.setGui(new GuiContainerVillager(player.inventory, new NpcTrader(player, merchant.getDisplayName(), (EntityTrader)player.world.getLoadedEntityList().stream().filter(e -> e.getUniqueID().toString().equals(uuid)).collect(Collectors.toList()).get(0)), player.world));
	}
